...
int projectile_update(projectile_t* _prj, game_container* _enemy_container, player_t* _player){
    if(_prj == NULL)
	return -1;
    ...
    for(int i = 0; i < _enemy_container->count; i++){
	enemy_t* _enemy = _enemy_container->array[i];
